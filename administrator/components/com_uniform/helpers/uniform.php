...
class JSNUniformHelper
{
	...
	public static function addAssets()
	{
		...
		$stylesheets[] = JURI::root(true) . '/plugins/system/jsnframework/assets/3rd-party/bootstrap/css/bootstrap.min.css';
	...
	public static function addToolbarMenu()
	{
		...
		JSNMenuHelper::addEntry('toolbar-menu', 'Menu', '', false, 'jsn-icon32 jsn-icon-menu', 'toolbar');
	...
	{
		// Create a toolbar button that drop-down a sub-menu when clicked
		JSNMenuHelper::addEntry('toolbar-new-form', JText::_('JTOOLBAR_NEW'), '', false, 'jsn-icon32 jsn-icon-plus', 'toolbar');
		...
		if ($items)
		{
			...
			foreach ($formPages as $i => $contentForm)
			{
				...
				if ($i + 1 == count($formPages))
				{
					...
					elseif (!empty($items->form_captcha) && $items->form_captcha == 2)
					{
						...
						$html .= '<div class="control-group">
								...
								<input type="text" id="jsn-captcha" class="jsn-input-small-fluid" autocomplete="off" name="captcha" placeholder="' . JText::_("JSN_UNIFORM_CAPTCHA") . '">
								</div>
								</div>';
					}
				}
			...
			foreach ($getHeadData['scripts'] as $script => $option)
			{
				if ($script == JSN_UNIFORM_ASSETS_URI . '/js/form.js')
				{
					...
					{
						$scripts[JSN_UNIFORM_ASSETS_URI . '/js/libs/jquery-ui-1.9.2.custom.min.js'] = $option;
				...
				if ($script == JSN_UNIFORM_ASSETS_URI . '/js/submissions.js' || $script == JSN_UNIFORM_ASSETS_URI . '/js/submission.js')
				{
					...
					$scripts[JSN_UNIFORM_ASSETS_URI . '/js/libs/bootstrap.min.js'] = $option;
			...
			if ($checkLoadScript)
			{
				...
				$document->addScript(JSN_UNIFORM_ASSETS_URI . '/js/libs/bootstrap.min.js');
				...
				if (!empty($checkLoadJS['date']))
				{
					$document->addScript(JSN_UNIFORM_ASSETS_URI . '/js/libs/jquery-ui-1.9.2.custom.min.js');
