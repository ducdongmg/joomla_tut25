...
abstract class JSNTplCompressCss
{
	...
	public static function compress ($htmlMarkup)
	{
		...
		if (isset($compressedFiles))
		{
			...
			$pageId    = JRequest::getInt('Itemid');

			if (file_exists($trackFile))
			{
				include $trackFile;

				if (isset($tracking) && isset($tracking[$pageId]) && isset($tracking[$pageId]['css']))
				{
					foreach ($tracking[$pageId]['css'] as $file)
					{
			...
			if ( ! isset($tracking))
			{
				$tracking = array($pageId => array());
			}

			$tracking[$pageId]['css'] = $compressedFiles;
